...
const CanvasElmt = ({
  ...
  const [width, setWidth] = useState(
    window.innerWidth < 400 ? window.innerWidth - 10 : 400
  );
  const [height, setHeight] = useState(
    window.innerWidth < 400 ? window.innerWidth - 10 : 420
  );
  //-----------------------------------------------
  useEffect(() => {
  ...
  function changeWidth({ target }) {
    if (target.value > window.innerWidth - 10) {
      inRotation();
      if (window.innerWidth > 500) {
        setWidth(window.innerWidth - 165);
        return;
      }
